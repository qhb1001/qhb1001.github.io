...
def totals_tree(m):
    ...
    if is_mobile(m):
        ans.append(totals_tree(end(left(m))))
        ans.append(totals_tree(end(right(m))))

    return tree(total_weight(m), ans)
...
def make_fib():
    ...
    def fib():
        nonlocal  fib1, fib2
        temp = fib1 # notice that this is value-pass
